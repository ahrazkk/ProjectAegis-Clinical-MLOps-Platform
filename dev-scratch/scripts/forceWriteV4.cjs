...
export default function KnowledgeGraphView({ drugs = [], result, polypharmacyResult, isMobile = false }) {
  ...
  const { nodes, edges, gridTiles, directPaths } = useMemo(() => {
    ...
    // Plot REAL user selected drugs
    if (drugs && drugs.length > 0) {
      drugs.forEach((d, idx) => {
        const pos = POSITIONS[idx % POSITIONS.length];
        const color = COLORS[idx % COLORS.length];
        // d is { name, smiles, drugbank_id, category, ... }
        addNode(d.name || \`Drug \${idx}\`, pos.q, pos.r, d, color);
      });
    }
    ...
    if (polypharmacyResult && polypharmacyResult.interactions) {
      ...
      interactions = [{
        drug_a: result.drug_a || drugs[0]?.name,
        drug_b: result.drug_b || drugs[1]?.name,
    ...
    // Process drawn interactions between nodes
    interactions.forEach(intx => {
      const srcName = (intx.drug_a || "").toLowerCase();
      const tgtName = (intx.drug_b || "").toLowerCase();
      
      const sourceNode = nList.find(n => n.name && n.name.toLowerCase() === srcName);
      const targetNode = nList.find(n => n.name && n.name.toLowerCase() === tgtName);
    ...
    return { nodes: nList, edges: eList, gridTiles: gTiles, directPaths: dPaths, hasAlert };
  ...
  return (
    ...
    >
      ...
      {hoveredNode && (
        ...
        >
          ...
          <div className="text-xs font-mono uppercase tracking-widest text-opacity-80 mb-3" style={{ color: hoveredNode.color }}>
            {hoveredNode.category || hoveredNode.drug_class || "CLINICAL PROFILE"}
          </div>
          
          <p className="text-sm text-gray-300 leading-relaxed mb-4">
            {hoveredNode.description || "Active substance currently mapped in Graph environment."}
          </p>
          
          <div className="p-3 rounded-lg bg-black/40 border border-white/5 space-y-2">
            <div>
              <span className="text-xs text-gray-500 uppercase tracking-wider block mb-1">Graph Relation</span>
              <span className="text-sm text-white font-medium">Selected Origin Drug</span>
            </div>
            {hoveredNode.smiles && (
               <div>
                  <span className="text-xs text-gray-500 uppercase tracking-wider block mt-2 mb-1">SMILES string</span>
                  <span className="text-[10px] text-gray-400 font-mono break-all">{hoveredNode.smiles.substring(0,60)}{hoveredNode.smiles.length > 60 ? '...' : ''}</span>
               </div>
            )}
          </div>
        </div>
      ...
      {/* Floating HUD */}
      <div className="absolute bottom-4 left-4 pointer-events-none bg-[#0a0a0f]/80 px-4 py-8 rounded-lg border border-[rgba(255,255,255,0.1)] backdrop-blur-md shadow-lg">
        <div className="flex flex-col gap-2">
          ...
          {drugs.map((d, i) => (
             <div key={i} className="flex items-center gap-2">
               ...
               <span className="text-[10px] text-gray-300 tracking-widest font-bold top-[1px]">{d.name.toUpperCase()}</span>
             </div>
          ))}
...
console.log('Successfully written KnowledgeGraphView.jsx with real data mapping, MOCK_DRUGS removed, zooming intact, and direct interaction conditional rules applied!');
