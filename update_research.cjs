...
// 1. Replace the Overview Rings
const oldRings = `<div className="grid grid-cols-2 gap-6">
...
const newRings = `<div className="grid grid-cols-2 xl:grid-cols-4 gap-6">
                        <ProgressRing value={98.3} color="#00d4ff" label="ROC-AUC" sublabel="GNN Model" />
                        <ProgressRing value={98.0} color="#00ff88" label="PR-AUC" sublabel="GNN Model" />
                        <ProgressRing value={99.4} color="#f59e0b" label="Recall" sublabel="Macro Graph" />
                        <ProgressRing value={81.1} color="#a855f7" label="F1 Score" sublabel="Macro Graph" />
                        {/* Note: Global Validation Accuracy is 98.67% and Precision is 68.47% due to class imbalance testing, achieving an impressive F1 of 81.1% on unseen data. */}
                      </div>`;

content = content.replace(oldRings, newRings);

// 2. Replace the Model Evolution Tab entirely
const evolutionStart = "{/* •••••••••••••••• MODEL EVOLUTION TAB •••••••••••••••• */}";
const evolutionEnd = "                </motion.div>\n              )}";

// Because UTF-8 bullet marks can be tricky, let's use indexOf and regex or just substring
const startIdx = content.indexOf('EVOLUTION TAB');
if(startIdx === -1) {
  console.log("Could not find EVOLUTION TAB");
} else {
  // Back up to the comment start
  const trueStart = content.lastIndexOf('{/*', startIdx);
  const endIdx = content.indexOf("</motion.div>\n              )}", trueStart);
  
  if(endIdx !== -1) {
    const trueEnd = endIdx + "</motion.div>\n              )}".length;
    
    // Construct new evolution block
    const newEvolution = `{/* •••••••••••••••• MODEL EVOLUTION TAB •••••••••••••••• */}
              {activeTab === 'evolution' && (
                <motion.div key="evolution" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="space-y-12">
                    ...
                    <Box glow="rgba(0,212,255,0.05)" className="!border-cyan-500/50 relative overflow-hidden group ml-12">
                      ...
                      <div className="flex flex-col xl:flex-row gap-6 lg:gap-12 items-start xl:items-center relative z-10">
                        ...
                        <div className="w-full xl:w-64 bg-cyan-500/5 rounded p-4 border border-cyan-500/30 space-y-4 shrink-0 shadow-[0_0_20px_rgba(0,212,255,0.05)]">
                           <div>
                              <div className="flex justify-between text-[10px] text-fui-gray-200 mb-1"><span>Accuracy</span> <span className="text-cyan-400 font-bold font-mono">98.27%</span></div>
    ...
    const pre = content.substring(0, trueStart);
    const post = content.substring(trueEnd);
    content = pre + newEvolution + post;
    
    fs.writeFileSync(path, content, 'utf8');
    console.log("Rewrote Model Evolution cleanly!");
  } else {
    console.log("Could not find trueEnd");
  }
}
