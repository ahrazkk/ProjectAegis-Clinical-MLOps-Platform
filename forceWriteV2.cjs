...
export default function KnowledgeGraphView({ drugs = [], result, polypharmacyResult, isMobile = false }) {
  ...
  const { nodes, edges, gridTiles, directPath } = useMemo(() => {
    ...
    // Path from A (-2,0) -> B (2,0) going AROUND the shared neighbor (0,0)
    // We route over the top: [-1,-1] -> [0,-1] -> [1,-1] -> [1,0]
    ...
    // Complete polyline node points: A -> empty path tiles -> B
      ...
      {hoveredNode && (
        <div 
          ...
          style={{
            ...
            left: dim.w/2 + pan.x + hoveredNode.x + 80,
            top: dim.h/2 + pan.y + hoveredNode.y - 80,
      ...
      <div className="absolute bottom-4 left-4 pointer-events-none bg-[#0a0a0f]/80 px-4 py-3 rounded-lg border border-[rgba(255,255,255,0.1)] backdrop-blur-md shadow-lg">
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-2 ml-2 pl-4 border-l border-gray-700">
            ...
            <span className="text-[10px] text-red-400 tracking-widest font-bold top-[1px]">DIRECT INTERACTION</span>
          </div>
        </div>
      </div>
...
console.log('KnowledgeGraphView successfully enhanced with tooltips and pathfinding!');
